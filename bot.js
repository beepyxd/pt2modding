...
client.on('message', async message => {
	...
	if (commandfile==="ping") {
		...
		.addField("pt2ci + JSON FILE", "Info checker")
		.addField("pt2space + TXT FILE", "Convert your path to space")
		.addField("pt2ping", "Bot's ping")
		.addField("pt2say", "Bot send your message")
		...
		https.get(Attachment, response => {
			...
			stream.on("finish", async () => {
				...
				function info(data, bpm1, bpm2, bpm3) {
					...
					if (res1[0]!=="NO ERRORS!"&&res1[1]!=="NO ERRORS!") return "**ID 1:**\n\n"+res1.join("\n\n");
					if (res1[0]!=="NO ERRORS!") return "**ID 1:**\n\n"+res1[0];
					if (res1[1]!=="NO ERRORS!") return "**ID 1:**\n\n"+res1[1];
					if (res2[0]!=="NO ERRORS!"&&res2[1]!=="NO ERRORS!") return "**ID 2:**\n\n"+res2.join("\n\n");
					if (res2[0]!=="NO ERRORS!") return "**ID 2:**\n\n"+res2[0];
					if (res2[1]!=="NO ERRORS!") return "**ID 2:**\n\n"+res2[1];
					if (res3[0]!=="NO ERRORS!"&&res3[1]!=="NO ERRORS!") return "**ID 3:**\n\n"+res3.join("\n\n");
					if (res3[0]!=="NO ERRORS!") return "**ID 3:**\n\n"+res3[0];
					if (res3[1]!=="NO ERRORS!") return "**ID 3:**\n\n"+res3[1];
	...
	} else if (commandfile==="space") {
		if (message.channel.type!=="dm") {if (message.channel.id!=="581184610067808298") return message.react(":whyhere:582157451848187904")}
		try {
			var Attachment = message.attachments.first().url;
		} catch(e) {
			return message.reply(":x: Put TXT file!");
		}
