...
module.exports = {
  check: function check_errors(path1, paths, multiplying) {
	...
	for (i=0; i<paths.length; i++) {
		...
		if (paths[i].indexOf("(")>0) {result=paths[i]}
		...
		else if (paths[i].includes("Q[")||paths[i].includes("R[")||paths[i].includes("S[")||paths[i].includes("T[")||paths[i].includes("U[")||paths[i].includes("V[")||paths[i].includes("W[")||paths[i].includes("X[")||paths[i].includes("Y[")) {result=paths[i]}
			...
			if (effects!=="{1}") {
				...
				for (j=0; j<paths_no_arp.length; j++) {
					if (spaces(paths_no_arp[j])==false) {result_tile=""}
					else {
						if (correct_notes.indexOf(paths_no_arp[j])==-1) {result=paths[i]} else {result_tile=""}
					}
				}
			}
		}
	...
	if (multiplying!=0) {
		...
		path1=path1_new;
		for (i=0; i<=path1.length; i++) {
			if (path1[i]=="-1") {result1=path1_return[i]}
		}
	}
	for (i=0; i<special_tiles.length; i++) {
	...
	if (result.length>0) {
		return `**CODE ERROR:** Error result: ${result}`;
	} else if (result_tile.length>0) {
		return `**CODE ERROR:** Error result: ${result_tile}`;
	} else if (result_tile1.length>0) {
		return `**CODE ERROR:** Error result: ${result_tile1}`;
	} else {
		return "NO ERRORS!";
	}
  }
